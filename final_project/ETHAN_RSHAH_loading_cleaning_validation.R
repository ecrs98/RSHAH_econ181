...
studio <- read.csv("Prelim Redfin Data/redfin_dc_studio.csv",
                   stringsAsFactors = F)
bed1_not_condo <- read.csv("Prelim Redfin Data/redfin_dc_1bed_not_condo.csv",
                           stringsAsFactors = F)
bed1_condo_0_750 <- read.csv("Prelim Redfin Data/redfin_dc_1bed_condo(0,750)sqft.csv",
                             stringsAsFactors = F)
bed1_condo_750_inf <- read.csv("Prelim Redfin Data/redfin_dc_1bed_condo(750,inf)sqft.csv",
                               stringsAsFactors = F)
bed2_not_condo <- read.csv("Prelim Redfin Data/redfin_dc_2bed_not_condo.csv",
                           stringsAsFactors = F)
bed2_condo_0_1250 <- read.csv("Prelim Redfin Data/redfin_dc_2bed_condo(0,1250)sqft.csv",
                              stringsAsFactors = F)
bed2_condo_1250_inf <- read.csv("Prelim Redfin Data/redfin_dc_2bed_condo(1250,inf).csv",
                                stringsAsFactors = F)
bed3_house_condo <- read.csv("Prelim Redfin Data/redfin_dc_3bed_house_condo.csv",
                             stringsAsFactors = F)
bed3_townhouse_0_1500 <- read.csv("Prelim Redfin Data/redfin_dc_3bed_townhouse(0,1500).csv",
                                  stringsAsFactors = F)
bed3_townhouse_1500_inf <- read.csv("Prelim Redfin Data/redfin_dc_3bed_townhouse(1500,inf).csv",
                                    stringsAsFactors = F)
bed3_all_else <- read.csv("Prelim Redfin Data/redfin_dc_3bed_all_else.csv",
                          stringsAsFactors = F)
bed4_not_townhouse <- read.csv("Prelim Redfin Data/redfin_dc_4bed_not_townhouse.csv",
                               stringsAsFactors = F)
bed4_townhouse <- read.csv("Prelim Redfin Data/redfin_dc_4bed_townhouse.csv",
                           stringsAsFactors = F)
bed5 <- read.csv("Prelim Redfin Data/redfin_dc_5bed.csv",
                 stringsAsFactors = F)
bed6plus <- read.csv("Prelim Redfin Data/redfin_dc_6plusbed.csv",
                     stringsAsFactors = F)
...
redfin_data[1, c(22, 21)] %>% 
  distHaversine(station_locations[1, c(5, 4)])
...
# testing for only the first property in the dataset. closest station is Dupont Circle,
# which is 664.9 m away from property.
# test_dist will become distance between first property and nth station.
# dist_vec is unnecessary for output but is useful for looking at output of all test_dist
for(n in 1:nrow(station_locations)) {
  
  test_dist <- as.matrix(redfin_data[1, c(22, 21)]) %>%
    distHaversine(as.matrix(station_locations[n, c(5, 4)]))
